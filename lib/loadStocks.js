...
export async function loadApiStockPrices(symbol) {
...
export async function loadFinnHubStockNews(symbol, daysBack) {
  ...
  const weekAgoDate = convertToTwoDigitDate(addDays(currentDate, daysBack))
...
export async function loadSingleStockTraders(symbol) {
  const response = await fetch(
    `http://localhost:3001/stocks/${symbol}/congress`
  )
  const { congressCounts } = await response.json()
  return { counts: congressCounts.slice(0, 21) }
}
